...
1. **Agent-runner core** — owns the poll loop, message formatting, DB reads/writes, MCP tool implementations, routing, status management, media handling. This is Paraclaw-specific and shared across all providers.
...
**Command detection:** Messages starting with `/` are checked against a command list. Recognized commands bypass formatting and are passed raw to the provider (for Claude's slash command handling) or intercepted by the agent-runner (for Paraclaw-level commands like session reset).
...
The agent-runner runs an MCP server that exposes Paraclaw tools to the agent. All tools write to the session DB.
