...
Host logs: `logs/paraclaw.log` (normal) and `logs/paraclaw.error.log` (errors only — some delivery/approval failures only show up here). **Log filename rename queued for 0.2.0** — kept as `paraclaw.log` through 0.1.x for operator-config back-compat (log-shippers, log-rotation rules).
